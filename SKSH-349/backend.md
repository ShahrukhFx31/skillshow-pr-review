...
| Import validator (`import-tool-row-validator.service`) calls `validateCreateEvent` — no parallel validation path | ✅ |
| UTC date parsing via existing `parseEventDatePayload` + UTC `dayjs` in range helper | ✅ |
| `EVENT_IMPORT_MAX_PAST_YEARS` name says "import" but applies to API create/update too | ✅ Accepted (comment documents create/import intent) |
...
- City rule is intentionally stricter than `eventName` (letter-only vs letter-or-digit) — matches ticket tests.

...
**PR comment (line 15):** `CREW_PERFORMANCE_REVIEWS_PAGE_SIZE = 10` is still duplicated in `use-crew-performance-reviews.ts` line 6 (API `limit`) and here line 144 (`pagination.pageSize`). Export once from `performance-reviews.constants.ts` and import in both files so page size cannot drift.
...
Impact:
- A one-sided edit yields wrong page counts or partial pages.
...
**Re-review:** Still **Open** at `d2236ca1` — constant still local in component (line 15) and hook (line 6); not exported from `performance-reviews.constants.ts`.
...
**Merge readiness:** **Approve with minor fix** — no Critical/High frontend blockers; 1 Medium DRY item (page-size constant). **Blocked by backend** until 2 open High findings are resolved.

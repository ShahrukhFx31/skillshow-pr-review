...
**Head:** `78463898948c899f6d5c707a29283a67908e63de` (unchanged since prior review)  
...
Recommendation:
Ship a matching backend change (or same ticket) to include `slug` on operational events (from `EventModel`) and crew `seq` (lookup by editor `id`) in the super-admin dashboard payload, then map `slug` in `useSuperAdminDashboard` the same way as `seq`. Until then, document as blocked on API or defer link renderers.
---
...
**Merge readiness:** Still blocked on open **High** — align super-admin API with `seq` / `slug` or scope link UI to a follow-up with API. (Re-verify 2026-07-21: no change.)
